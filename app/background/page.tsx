...
export const metadata: Metadata = {
  ...
  description: "Explore the authorship, date, audience, literary form, biblical world, structure, and purpose of James."
};
  ...
  if (block.type === "subsection") {
    return (
      <section className="background-subsection">
        <h3>{block.title}</h3>
        {block.body.map((paragraph, index) => <p key={`${block.title}-${index}`}>{paragraph}</p>)}
      </section>
    );
  }
  return (
...
export default function BackgroundPage() {
  ...
  return (
    <main className="background-page james-background-page">
      <section className="background-hero" aria-labelledby="background-title">
        <div className="background-hero-copy">
          <h1 id="background-title" aria-label={content.title}>
            ...
            <span className="background-title-book">James</span>
          </h1>
      ...
      <section className="background-shell" aria-label="Introduction to James">
