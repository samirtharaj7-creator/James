...
export function ScriptureReferenceChip({
  ...
  const href = getLocalJamesReferenceHref(reference);
...
function getLocalJamesReferenceHref(reference: string) {
  const match = reference.trim().match(/^James\s+(\d+):(\d+)(?:[-–—]\d+)?$/iu);
  if (!match) return null;

  return `/james/${match[1]}/#v${match[2]}`;
}
